...
TEST(Teleportation, RecoversStateFidelityOne) {
    ...
    const double cases[][2] = {
        {0.0, 0.0}, {M_PI / 2, 0.0}, {M_PI / 2, M_PI / 2},
...
TEST(Shor, Factor15) {
    ...
    for (auto f : r.factors) { if (f == 3) has3 = true; if (f == 5) has5 = true; }
...
TEST(Shor, Factor21) {
    ...
    bool ok = false;
    for (auto f : r.factors) if (f == 3 || f == 7) ok = true;
    EXPECT_TRUE(ok);
}
...
TEST(Shor, TrivialFactorWhenGcdNotOne) {
    ...
    ASSERT_TRUE(r.success);
    bool has3 = false; for (auto f : r.factors) if (f == 3) has3 = true;
    EXPECT_TRUE(has3);
}
...
TEST(Shor, CountingProbabilitiesNormalized) {
    shor::run(15, 7, [](int, bool, const Eigen::VectorXd& probs) {
        if (probs.size() > 1)
            ASSERT_NEAR(probs.sum(), 1.0, 1e-9);
    });
}
