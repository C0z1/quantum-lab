...
export class Chart2D {
  ...
  _resize() {
    ...
    const w = this.canvas.clientWidth || 320,
      h = this.canvas.clientHeight || 160;
  ...
  line(points, { color = COL.line } = {}) {
    ...
    grad.addColorStop(1, 'rgba(67,224,160,0)');
    ctx.beginPath();
    ctx.moveTo(X(points[0].x), y0);
    points.forEach((p) => ctx.lineTo(X(p.x), Y(p.y)));
    ctx.lineTo(X(points[points.length - 1].x), y0);
    ctx.closePath();
    ctx.fillStyle = grad;
    ctx.fill();
    // línea
