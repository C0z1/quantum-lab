...
static void inverseQFT(QuantumStateVector& sv, int start, int t) {
    ...
    for (int i = 0; i < t / 2; ++i)
        sv.applySwap(start + i, start + t - 1 - i);
    for (int j = 0; j < t; ++j) {
        for (int k = 0; k < j; ++k) {
...
static uint64_t orderFromPhase(uint64_t m, uint64_t M, uint64_t a, uint64_t N) {
    ...
    while (den != 0) {
        ...
        if (k_next > 0 && k_next <= N && powmod(a, k_next, N) == 1)
            return k_next;
        h_prev = h_cur; h_cur = h_next;
        k_prev = k_cur; k_cur = k_next;
        uint64_t r = num % den; num = den; den = r;
        if (k_cur > N) break;
    }
    return 0;
}
...
Result run(uint64_t N, uint64_t a, const FrameCallback& on_frame) {
    ...
    std::sort(idx.begin(), idx.end(),
              [&](int p, int q){ return counting(p) > counting(q); });
    ...
    if (r == 0) { res.success = false; return res; }
    res.order = r;
