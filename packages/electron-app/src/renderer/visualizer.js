...
export class QuantumVisualizer {
  constructor(canvas) {
    ...
    this.camera = new THREE.PerspectiveCamera(52, canvas.clientWidth / canvas.clientHeight, 0.1, 400);
    ...
    this.bloom = new UnrealBloomPass(new THREE.Vector2(canvas.clientWidth, canvas.clientHeight), 0.7, 0.5, 0.8);
    ...
    this.bars = []; this.targetProbs = []; this.stateSize = 0; this.targetIndex = -1;
    this.beams = []; this.rings = []; this.bursts = [];
    this.blochQubits = []; this.channels = []; this.nBloch = 0; this._burstDone = false;
  ...
  _addLights() {
    ...
    const key = new THREE.DirectionalLight(0xe6dcff, 0.65); key.position.set(6, 16, 8); this.scene.add(key);
    const p1 = new THREE.PointLight(this.C.iris, 1.2, 80); p1.position.set(-16, 7, -6); this.scene.add(p1);
    const p2 = new THREE.PointLight(this.C.saffron, 0.7, 80); p2.position.set(16, 6, 8); this.scene.add(p2);
  }

  _addStarfield() {
    const N = 2400;
    const pos = new Float32Array(N * 3), col = new Float32Array(N * 3);
    const palette = [new THREE.Color(this.C.iris), new THREE.Color(this.C.saffron), new THREE.Color(this.C.teal), new THREE.Color(0xffffff)];
    for (let i = 0; i < N; i++) {
      const r = 40 + Math.random() * 120, th = Math.random() * Math.PI * 2, ph = Math.acos(2 * Math.random() - 1);
    ...
    this.stars = new THREE.Points(g, new THREE.PointsMaterial({
      size: 1.7, map: PARTICLE_TEX, vertexColors: true, transparent: true, opacity: 0.8,
  ...
  _addFloor() {
    try {
      this.mirror = new Reflector(new THREE.PlaneGeometry(220, 220), {
        clipBias: 0.003, textureWidth: 1024, textureHeight: 1024, color: 0x0a1526,
      });
      this.mirror.rotation.x = -Math.PI / 2; this.mirror.position.y = -0.02;
  ...
  initBars(stateSize) {
    ...
    this._clearBeams();
    this.bars = []; this.targetProbs = new Array(stateSize).fill(0); this.stateSize = stateSize;
    ...
    for (let i = 0; i < stateSize; i++) {
      ...
      const mat = new THREE.MeshStandardMaterial({
        color: 0x0a0a12, emissive: new THREE.Color(0x2a1e55), emissiveIntensity: 1.1,
        metalness: 0.5, roughness: 0.3,
      });
      const bar = new THREE.Mesh(geo, mat);
      bar.position.set(-totalWidth / 2 + i * spacing, 0.5, 0);
      this.barsGroup.add(bar); this.bars.push(bar);
      if (showLabels) {
        const label = this._label('|' + i.toString(2).padStart(Math.log2(stateSize), '0') + '⟩', '#8a8a95');
  ...
  setPeaks(indices) { this._setBeams(indices || [], this.C.iris); }
  ...
  _makeBeam(x, color) {
    const group = new THREE.Group(); group.position.x = x; this.barsGroup.add(group);
    ...
    const NP = 22;
    const pos = new Float32Array(NP * 3); const spd = new Float32Array(NP);
    for (let i = 0; i < NP; i++) {
      pos[i * 3] = (Math.random() - 0.5) * 0.35; pos[i * 3 + 1] = Math.random() * 4; pos[i * 3 + 2] = (Math.random() - 0.5) * 0.35;
      spd[i] = 1.5 + Math.random() * 2.5;
    }
    const pg = new THREE.BufferGeometry(); pg.setAttribute('position', new THREE.BufferAttribute(pos, 3));
    const pts = new THREE.Points(pg, new THREE.PointsMaterial({ color, size: 0.2, map: PARTICLE_TEX, transparent: true, opacity: 0.8, blending: THREE.AdditiveBlending, depthWrite: false }));
    group.add(pts);
  ...
  pulse(originX = 0) { this._spawnRing(originX, this.targetIndex >= 0 ? this.C.saffron : this.C.iris); }

  _tickBars(dt, t) {
    const nStates = this.stateSize || 1;
    for (let i = 0; i < this.bars.length; i++) {
      const bar = this.bars[i]; const prob = this.targetProbs[i] || 0;
    ...
    for (const b of this.beams) {
      ...
      let bar = null, best = 1e9;
      for (const bb of this.bars) { const d = Math.abs(bb.position.x - bx); if (d < best) { best = d; bar = bb; } }
      const H = Math.max(0.6, (bar ? bar.scale.y : 1) + 1.6);
      b.cyl.scale.y = H; b.cyl.position.y = H / 2;
  ...
  initBloch(nQubits, labels) {
    ...
    this.blochQubits = []; this.channels = []; this.nBloch = nQubits; this._burstDone = false;
    const R = 1.7, spacing = 5.2, totalW = (nQubits - 1) * spacing;
    const centers = [];
    for (let q = 0; q < nQubits; q++) {
      ...
      root.add(new THREE.Mesh(new THREE.SphereGeometry(R, 30, 20),
        new THREE.MeshBasicMaterial({ color: 0x2b4a7a, wireframe: true, transparent: true, opacity: 0.3 })));
      root.add(new THREE.Mesh(new THREE.SphereGeometry(R * 0.99, 32, 24),
        new THREE.MeshBasicMaterial({ color: 0x0a1830, transparent: true, opacity: 0.25 })));
      // ecuador brillante
      const eq = new THREE.Mesh(new THREE.TorusGeometry(R, 0.02, 8, 60),
        new THREE.MeshBasicMaterial({ color: this.C.iris, transparent: true, opacity: 0.6, blending: THREE.AdditiveBlending }));
      ...
      const arrow = new THREE.ArrowHelper(new THREE.Vector3(0, 1, 0), new THREE.Vector3(), R, this.C.teal, 0.42, 0.26);
      ...
      label.position.set(0, -R - 0.7, 0); root.add(label);
      this.blochGroup.add(root);
      this.blochQubits.push({ root, arrow, R, currentDir: new THREE.Vector3(0, 1, 0), targetDir: new THREE.Vector3(0, 1, 0), targetLen: 1 });
    }
    // Canales de entrelazamiento (partículas fluyendo) entre qubits adyacentes.
    for (let q = 0; q < nQubits - 1; q++) this.channels.push(this._makeChannel(centers[q], centers[q + 1]));
  ...
  _makeChannel(a, b) {
    ...
    // tubo tenue
    const tube = new THREE.Mesh(new THREE.TubeGeometry(curve, 40, 0.03, 8, false),
      new THREE.MeshBasicMaterial({ color: this.C.iris, transparent: true, opacity: 0.32, blending: THREE.AdditiveBlending, depthWrite: false }));
    this.blochGroup.add(tube);
    const NP = 46;
    const pos = new Float32Array(NP * 3); const u = new Float32Array(NP);
    for (let i = 0; i < NP; i++) u[i] = Math.random();
    const pg = new THREE.BufferGeometry(); pg.setAttribute('position', new THREE.BufferAttribute(pos, 3));
    const pts = new THREE.Points(pg, new THREE.PointsMaterial({ color: 0xb79bff, size: 0.3, map: PARTICLE_TEX, transparent: true, opacity: 0.95, blending: THREE.AdditiveBlending, depthWrite: false }));
  ...
  _tickBloch(dt, t) {
    ...
    for (const ch of this.channels) {
      ...
      for (let i = 0; i < ch.NP; i++) {
        ch.u[i] += dt * 0.35; if (ch.u[i] > 1) ch.u[i] -= 1;
        const pt = ch.curve.getPoint(ch.u[i]);
        p.array[i * 3] = pt.x; p.array[i * 3 + 1] = pt.y; p.array[i * 3 + 2] = pt.z;
      }
      p.needsUpdate = true;
    }
  }
  ...
  // ------------------------------------------------------------------ FX
  _spawnRing(x, color) {
    const mesh = new THREE.Mesh(new THREE.RingGeometry(0.6, 0.9, 48),
      new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.8, blending: THREE.AdditiveBlending, side: THREE.DoubleSide, depthWrite: false }));
    mesh.rotation.x = -Math.PI / 2; mesh.position.set(x, 0.05, 0);
    this.fxGroup.add(mesh); this.rings.push({ mesh, age: 0, life: 1.3 });
  }

  _spawnBurst(center, color) {
    const NP = 140; const pos = new Float32Array(NP * 3); const vel = [];
    for (let i = 0; i < NP; i++) {
    ...
    const pts = new THREE.Points(g, new THREE.PointsMaterial({ color, size: 0.42, map: PARTICLE_TEX, transparent: true, opacity: 1, blending: THREE.AdditiveBlending, depthWrite: false }));
    this.fxGroup.add(pts); this.bursts.push({ pts, vel, NP, age: 0, life: 1.4, center });
  }

  _animateFX(dt, t) {
    if (this.stars) this.stars.rotation.y += dt * 0.012;
    for (let k = this.rings.length - 1; k >= 0; k--) {
      const r = this.rings[k]; r.age += dt;
      const s = 1 + r.age * 6; r.mesh.scale.set(s, s, s);
      r.mesh.material.opacity = Math.max(0, 0.65 * (1 - r.age / r.life));
      if (r.age > r.life) { this.fxGroup.remove(r.mesh); r.mesh.geometry.dispose(); r.mesh.material.dispose(); this.rings.splice(k, 1); }
    }
    for (let k = this.bursts.length - 1; k >= 0; k--) {
      const b = this.bursts[k]; b.age += dt; const p = b.pts.geometry.attributes.position;
      for (let i = 0; i < b.NP; i++) {
        p.array[i * 3] += b.vel[i].x * dt; p.array[i * 3 + 1] += b.vel[i].y * dt; p.array[i * 3 + 2] += b.vel[i].z * dt;
        b.vel[i].multiplyScalar(0.94);
      }
      p.needsUpdate = true; b.pts.material.opacity = Math.max(0, 1 - b.age / b.life);
      if (b.age > b.life) { this.fxGroup.remove(b.pts); b.pts.geometry.dispose(); b.pts.material.dispose(); this.bursts.splice(k, 1); }
    }
  }

  // ------------------------------------------------------------------ util
  _axis(dir, R, color) {
    const g = new THREE.BufferGeometry().setFromPoints([dir.clone().multiplyScalar(-R), dir.clone().multiplyScalar(R)]);
    return new THREE.Line(g, new THREE.LineBasicMaterial({ color, transparent: true, opacity: 0.4 }));
  }

  _label(text, color) {
    const c = document.createElement('canvas'); c.width = 170; c.height = 64;
    const ctx = c.getContext('2d'); ctx.fillStyle = color; ctx.font = 'bold 26px monospace'; ctx.textAlign = 'center';
    ...
    spr.scale.set(1.9, 0.72, 1); return spr;
  }

  _clearBeams() {
    for (const b of this.beams) { this.barsGroup.remove(b.group); b.group.traverse((o) => { o.geometry?.dispose?.(); o.material?.dispose?.(); }); }
    this.beams = [];
  }

  _clearGroup(group) {
    while (group.children.length) {
      const c = group.children[0]; group.remove(c);
      c.traverse?.((o) => { o.geometry?.dispose?.(); if (o.material) { o.material.map?.dispose?.(); o.material.dispose?.(); } });
    }
  }
  ...
  _startRenderLoop() {
    const loop = () => {
      ...
      if (this.mode === 'bars') this._tickBars(dt, t); else this._tickBloch(dt, t);
  ...
  reset() {
    for (const b of this.bars) { b.scale.y = 0.02; b.position.y = 0.01; }
...
export function computeBloch(data, stateSize, q) {
  const stride = 1 << q; let r00 = 0, r11 = 0, r01re = 0, r01im = 0;
  for (let i = 0; i < stateSize; i++) {
    if (i & stride) continue;
    const j = i | stride;
    const are = data[i * 3], aim = data[i * 3 + 1];
    const bre = data[j * 3], bim = data[j * 3 + 1];
    r00 += are * are + aim * aim; r11 += bre * bre + bim * bim;
    r01re += are * bre + aim * bim; r01im += aim * bre - are * bim;
  }
