...
void QuantumStateVector::applyHadamard(int qubit) {
    if (qubit < 0 || qubit >= n_qubits_) throw std::out_of_range("qubit index out of range");
...
void QuantumStateVector::applyToffoli(int c1, int c2, int target) {
    ...
    if (c1 < 0 || c1 >= n_qubits_ || c2 < 0 || c2 >= n_qubits_ || target < 0 || target >= n_qubits_)
        throw std::out_of_range("qubit index out of range");
...
void QuantumStateVector::applyRZ(int qubit, double theta) {
    ...
    const int64_t stride = int64_t(1) << qubit;
    const int64_t n = dim();
    for (int64_t i = 0; i < n; ++i) sv_(i) *= (i & stride) ? p1 : p0;
}
...
void QuantumStateVector::applyControlledModMul(int control_qubit, int work_start, int work_qubits,
    ...
    for (int64_t i = 0; i < n; ++i) {
        ...
        const int64_t i2 = (i & ~work_mask) | (static_cast<int64_t>(y2) << work_start);
        out(i2) = sv_(i);
    }
    sv_ = out;
}
...
Eigen::MatrixXcd QuantumStateVector::kronecker(const Eigen::MatrixXcd& A,
    ...
    for (int i = 0; i < ar; ++i)
        for (int j = 0; j < ac; ++j) K.block(i * br, j * bc, br, bc) = A(i, j) * B;
    return K;
}

Eigen::MatrixXcd QuantumStateVector::gateMatrix(int qubit, const Eigen::MatrixXcd& gate) const {
